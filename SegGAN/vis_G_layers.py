...
def vis_features_layers(feature_conv,th=40,t=24,layers=18,set=False):
    ...
    if set:
        name = './test/result_layers/netG_trainval/DownS'+ str(t) +'_heat_'+ str(layers) +'.jpg'
    else:
        name = './test/result_layers/netG_trainval/DownS'+ str(t) +'_heat_'+ str(layers) +'.jpg'
...
G_path = "/home/jjchu/My_Research/SegGAN/checkpoints/experiment_cgan_strided_trainval/200_net_G.pth"
D_path = "/home/jjchu/My_Research/SegGAN/checkpoints/experiment_cgan_strided_trainval/200_net_D.pth"
...
t = 1
# for t in range(1,10):
imgpath = './test/result_layers/netG_trainval/' + str(t) + 'real.jpg'
fakepath = './test/result_layers/netG_trainval/' + str(t) + 'fake.jpg'
# layers = [1,3,4,6,8,9,11,13,15]
# for layer in layers:
layer=2
features_blobs = []
net.netG.module.model[0].DownsampleStrided[layer].register_forward_hook(hook_feature)
...
save_generate_fig(fake_img[0],fakepath)
saveFig(real_img[0],imgpath)
feature_conv = features_blobs[0] # (1,3,256,256)
img = cv2.imread(imgpath)
height, width, _ = img.shape
MAPs = vis_features_layers(feature_conv[0],th,t,layer,False)
