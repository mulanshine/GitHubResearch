...
class CGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        # self.loss_names = ['D_real','D_fake', 'loss_D_cls','loss_D_grap','G_fake','G_cls']
    ...
    def backward_D(self):
        ...
        # Fake
        self.fake_img = self.netG(self.real_img, self.real_label)
        # self.fake_img_pool = self.fake_pool.query(self.fake_img)
        self.pred_fake, self.pred_fake_cls = self.netD(self.fake_img)
        # self.pred_fake, self.pred_fake_cls = self.netD(self.fake_img_pool.detach())
        self.loss_D_fake = self.criterionGAN(self.pred_fake, False)
        # combined loss
        self.loss_D = (self.lambda_real * self.loss_D_real + self.lambda_fake * self.loss_D_fake)* 0.5 + self.lambda_cls * self.loss_D_cls 
        self.loss_D.backward()
        # self.loss_D_fake_cls = self.criterionCls(self.pred_fake_cls, self.real_label)
        # self.loss_D_cls = (self.loss_D_real_cls + self.loss_D_fake_cls) * 0.5
