...
from data.spinal_cord_crop_dataset import spinalcordRealCropDataSet,spinalcordGen128DataSet
import scipy
...
def threshold_predictions(predictions, thr=0.9):
    thresholded_preds = predictions[1]
    low_values_indices = thresholded_preds < thr
    thresholded_preds[low_values_indices] = 0
    low_values_indices = thresholded_preds >= thr
    thresholded_preds[low_values_indices] = 1
    predictions[1,::] = thresholded_preds
...
def test_net_dice(args, net, batch_size=1, gpu=False,site=["site3","site3"]):
    net.eval()
    train_dataset = spinalcordGen128DataSet(args.spinal_root,img_size=args.img_size,site=site,batchsize=1, n_class=args.n_class, nlabel=args.nlabel,set=args.set,real_or_fake='real')
    ...
    for batch_idx, (data, target, label, size, name) in enumerate(train_loader):
        ...
        # pred_1 = np.argmax(masks_pred[0],0)
        pred = threshold_predictions(masks_pred[0], thr=0.9)
...
def train_net(args, net,epochs=5,batch_size=1,lr=0.1,val_percent=0.05,save_cp=True,gpu=False):
    ...
    for epoch in range(epochs):
        ...
        train_dataset_site1 = spinalcordGen128DataSet(args.spinal_root,img_size=args.img_size,site=['site1','site1'],batchsize=args.batchsize, n_class=args.n_class, nlabel=args.nlabel,set=args.set,real_or_fake=args.real_or_fake)
        train_dataset_site2 = spinalcordGen128DataSet(args.spinal_root,img_size=args.img_size,site=['site2','site2'],batchsize=args.batchsize, n_class=args.n_class, nlabel=args.nlabel,set=args.set,real_or_fake=args.real_or_fake)
        # train_dataset_site1 = spinalcordRealCropDataSet(args.spinal_root,img_size=args.img_size,site=['site1','site1'],batchsize=args.batchsize, n_class=args.n_class, nlabel=args.nlabel,set=args.set)
        # train_dataset_site2 = spinalcordRealCropDataSet(args.spinal_root,img_size=args.img_size,site=['site2','site2'],batchsize=args.batchsize, n_class=args.n_class, nlabel=args.nlabel,set=args.set)
        train_loader_site1 = torch.utils.data.DataLoader(train_dataset_site1, batch_size=args.batchsize, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)
        train_loader_site2 = torch.utils.data.DataLoader(train_dataset_site2, batch_size=args.batchsize, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)

        for i_iter, _ in enumerate(range(args.num_steps)):
            if i_iter % 2 == 0:   # 4:1
                # c++: next --> __next__
                _, batch = enumerate(train_loader_site2).__next__()
            else:
                _, batch = enumerate(train_loader_site1).__next__()

            data, target, label, size, name = batch
            # print(name)

        # for batch_idx, (data, target, label, size, name) in enumerate(train_loader_site2):
        #     batch_idx = batch_idx + 1
...
def get_args():
    ...
    parser.add_option("--img_size", default=(128,128)) #(128，128)‘
    ...
    # parser.add_option("--logfile", default="/home/jjchu/Result/UNetsnapshots/Real_center100_mixedsite12_meanstd_imgs_b8_25l_1103/log.txt",type=str)
    # parser.add_option("--snapshots", default="/home/jjchu/Result/UNetsnapshots/Real_center100_mixedsite12_meanstd_imgs_b8_25l_1103/",type=str)
    # parser.add_option("--savepath", default="/home/jjchu/Result/UNetResults/Real_center100_mixedsite12_meanstd_imgs_b8_25l_1103/",type=str)  
    # parser.add_option('-c', '--load', default="/home/jjchu/Result/UNetsnapshots/Real_center100_mixedsite12_meanstd_imgs_b8_25l_1103/CP60.pth",help='load file model')
    ...
    parser.add_option("--spinal_root", default="/home/jjchu/Result/GANUNetResults/lblrescgan_center100_site12_lbl2img_gansite3_10L1bg_10PER_1GAN_20GWMPER_unet128_batch8_step200/test_latest/images/", type=str)
    parser.add_option("--logfile", default="/home/jjchu/Result/UNetsnapshots/lblrescgan_mixed_kernel7_center100_site12_lbl2img_gansite3_10L1bg_10PER_1GAN_20GWMPER_unet128_batch8_step200_real_fake_b12_25l_1103/log.txt",type=str)
    parser.add_option("--snapshots", default="/home/jjchu/Result/UNetsnapshots/lblrescgan_mixed_kernel7_center100_site12_lbl2img_gansite3_10L1bg_10PER_1GAN_20GWMPER_unet128_batch8_step200_real_fake_b12_25l_1103/",type=str)
    parser.add_option("--savepath", default="/home/jjchu/Result/UNetResults/lblrescgan_mixed_kernel7_center100_site12_lbl2img_gansite3_10L1bg_10PER_1GAN_20GWMPER_unet128_batch8_step200_real_fake_b12_25l_1103/",type=str)    
    parser.add_option('-c', '--load', default="/home/jjchu/Result/UNetsnapshots/lblrescgan_mixed_kernel7_center100_site12_lbl2img_gansite3_10L1bg_10PER_1GAN_20GWMPER_unet128_batch8_step200_real_fake_b12_25l_1103/CP30.pth",type=str)
    parser.add_option("--train_list", default="./data/train_site12_list.txt", type=str)
    ...
    parser.add_option("--num_steps",default=56,type=int)
