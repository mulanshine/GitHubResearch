...
class ImgCGANDataset(BaseDataset):
    @staticmethod
    ...
    def __getitem__(self, index):
        ...
        real = (image-0.5)*2
        normimg, target, label = self.transtype(image, label)
        aff = self.compute_relation_map(image,denoising=self.opt.affdenoising)
        return {'img': image,'real':real,'aff':aff,'target':target, 'label':label, 'path': maskpath}

    def transtype(self,image, mask):
        # for image
        normimg = (image - image.min()) / (image.max()-image.min()+1e-10) * 2.0
        normimg = normimg - 1.0
        normimg = torch.FloatTensor(normimg)
        # for target
        lbls = self.crop_GM_WM(mask)
        target = normalize_slice(image, lbls, self.val_gm, self.val_bk)
        # target = mean_std_norm_slice(imgL, lbls, self.val_gm, self.val_wm, self.std_gm, self.std_wm)
        target = (target - 0.5) * 2
        target = torch.FloatTensor(target)
        # for mask
        mask = torch.FloatTensor(mask)
        mask = mask.long()
        return normimg, target, mask
