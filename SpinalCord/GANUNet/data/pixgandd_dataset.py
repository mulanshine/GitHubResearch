...
class PixGANDDDataset(BaseDataset):
    @staticmethod
    ...
    def initialize(self, opt):
        ...
        self.imgpaths12 = sorted(make_dataset(self.imgdir,site=['site1','site2']))
        self.imgpaths34 = sorted(make_dataset(self.imgdir,site=['site3','site4']))
        self.valid_classes = [0, 128, 255]
        self.train_classes = [0, 1, 2]
        self.class_map = dict(zip(self.valid_classes, self.train_classes))
        self.val_gm = 0.56590 # 131.5549/255.0=0.515901568627451+0.05
        self.val_wm = 0.45518 # 116.0709/255.0
        self.std_gm = 0.03399
        self.std_wm = 0.04475

    def __getitem__(self, index):
        imgpath12 = self.imgpaths12[index]
        imgpath34 = self.imgpaths34[index]
        mask12, imgL12 = self.get_imgmask(imgpath12)
        mask34, imgL34 = self.get_imgmask(imgpath34)
        imgRGB12 = self.get_RGBimg(imgL12)
        imgRGB34 = self.get_RGBimg(imgL34)
        target12, mask12 = self.transtype(imgL12, mask12)
        aff12 = self.compute_relation_map(imgRGB12)
        return {'img12': imgRGB12,'img34': imgRGB34, 'target12':target12, 'aff12':aff12,'path': imgpath12}

    def get_imgmask(self,imgpath):
    ...
    def transtype(self,imgL, mask):
        ...
        # img = normalize_slice(imgL, lbls, self.val_gm, self.val_wm)
        img = mean_std_norm_slice(imgL, lbls, self.val_gm, self.val_wm, self.std_gm, self.std_wm)
        img = np.expand_dims(img, axis=0)
        imgRGB = np.concatenate((img,img,img),axis=0)
        imgRGB = (imgRGB - 0.5) * 2
    ...
    def __len__(self):
        return len(self.imgpaths12)

    def name(self):
        return 'PixGANDDDataset'
